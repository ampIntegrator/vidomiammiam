...
import { createBlogPosts } from './seed/blog-posts-10'
...
export const seedBlogPosts: PayloadHandler = async (req) => {
  ...
  try {
    ...
    const demoAuthor = await payload.find({
      collection: 'users',
      where: {
        email: {
    ...
    if (!demoAuthor.docs.length) {
      demoAuthor.docs[0] = await payload.create({
        collection: 'users',
        data: {
          name: 'Demo Author',
    ...
    const author = demoAuthor.docs[0]
    ...
    payload.logger.error(`Error seeding blog posts: ${error.message}`)
    return Response.json({
      success: false,
      error: error.message,
    }, { status: 500 })
